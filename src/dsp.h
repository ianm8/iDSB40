...
static const int16_t __not_in_flash_func(process_iDSB)(const int16_t s)
{
  ...
  const int32_t mic = lpf_2400(hpf_300(s));
...
// envelope delay taps 32us each
// sweep 0..5 on air
#define ENV_DELAY_TAPS 1

static const int16_t __not_in_flash_func(process_SSB)(const int16_t s,int16_t &frequency,const uint8_t env_delay = ENV_DELAY_TAPS)
{
  volatile static int32_t alc = 256L;
  ...
  // remove DC and band limit mic signal
  const int16_t mic_in = lpf_SSB_2400(hpf_300(s));
  ...
  // wrap to [-fs/2, +fs/2) and
  // allow for 180 degree postive and
  // negative phase shifts at zero
  // envelope amplitude
  if (freq >= ONE__PI) freq -= TWO__PI;
  else if (freq < -ONE__PI) freq += TWO__PI;
  frequency = constrain(freq, -8000, 8000);;

  // get magnitude
  int32_t magnitude = isqrt((int32_t)sig_i*sig_i + (int32_t)sig_q*sig_q);

  // envelope delay: 1 tap = 32us
  static int16_t mag_dly[16] = {0};
  static uint8_t mp = 0;
  mag_dly[mp & 15] = magnitude;
  magnitude = mag_dly[(uint8_t)(mp - env_delay) & 15];
  mp++;

  // ALC
  magnitude = (magnitude * alc) >> 8;
