...
class Si5351mcu {
    private:
        ...
        uint8_t  force_div[SICHANNELS] = { 0 };
    ...
    public:
        ...
        // allow glitch free for a limited range
        void forceDivider(uint8_t clk, uint16_t div) { force_div[clk] = div; }

        // reset all PLLs
        static void reset(void);

        // set CLKx(0..2) to freq (Hz)
        void setFreq(uint8_t, uint32_t, const bool = false);

        // fast, small, frequency change
        void setFreqTX(uint8_t clk, uint32_t freq);
