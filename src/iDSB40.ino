 ...
 *  1.0.240 iDSB, DSB
 *  2.1.240 CW, AM
 *  2.2.240 SSB - experimental
 *  3.1.240 Two-tone test
 *  3.2.240 SSB fixes
 *  3.3.240 CW gaussian
 *
 */
...
// uncomment to replace mic with test tones
//#define TEST_TWO_TONE  
...
#ifdef TEST_TWO_TONE
static int16_t __not_in_flash("fast_access_sram2") sine256[256];
static void init_two_tone(void)
{
  for (int i = 0; i < 256; i++)
  {
    sine256[i] = (int16_t)lroundf(1023.0f * sinf(2.0f * (float)M_PI * (float)i / 256.0f));
  }
}

static const int16_t __not_in_flash_func(two_tone)(void)
{
  // tuning word = f * 2^32 / 31250
  static uint32_t p1 = 0;
  static uint32_t p2 = 0;
  p1 += 96207267ul;   //  700 Hz
  p2 += 261134012ul;  // 1900 Hz
  const int16_t s1 = sine256[p1 >> 24];
  const int16_t s2 = sine256[p2 >> 24];
  return s1 + s2;     // peaks ±2046, matches full ADC swing
}
#endif
...
void setup(void)
{
  ...
  SI5351.forceDivider(0,122);
  SI5351.setPower(0,SIOUT_8mA);
  SI5351.setFreq(0,radio.frequency);
  SI5351.reset();
...
void update_display(const uint32_t signal_level = 0u,const bool show_step = false)
{
  ...
  if (debug_value!=0)
  {
    if (debug_value<1000000)
    {
      debug_value += 1000000;
    }
    ultoa(debug_value,sz_frequency,10);
  }
  else
  {
  ...
  switch (radio.mode)
  {
    case MODE_SSB:  oled.print(radio.tx_enable?" *SSB":"  SSB"); break;
...
volatile static int16_t frequency_delta = 0;
      ...
      if (adc_value_ready)
      {
        ...
        switch (radio.mode)
        {
          ...
          case MODE_SSB:
          {
            ...
            volatile static int32_t df_raw = 0;
            int16_t df = 0;
            tx_value = tx_old;
#ifdef TEST_TWO_TONE
            static uint8_t env_delay = 0;
            switch (r.process())
            {
              case DIR_CW:
              {
                if (env_delay<5) env_delay++;
                break;
              }
              case DIR_CCW:
              {
                if (env_delay>0) env_delay--;
                break;
              }
            }
            tx_raw += process_SSB(two_tone(),df,env_delay);
            debug_counter = env_delay;
#else
...
static void process_mic(void)
{
  ...
  {
    SI5351.setFreq(0,radio.frequency-2700);
  }
  else
  {
    SI5351.setFreq(0,radio.frequency);
  ...
  while (digitalRead(PIN_PTT)==LOW)
  {
    if (radio.mode==MODE_SSB)
    {
      if (frequency_ready)
      {
        ...
        const uint32_t f = (uint32_t)((int32_t)radio.frequency - (int32_t)frequency_delta);
        SI5351.setFreq(0, f, true);
      }
      // we need to update the frequency as
      // rapidly as possible, so don't update
      // the display, just continue
      continue;
    }
...
void loop1(void)
{
  ...
  if (!radio.tx_enable)
  {
    ...
    if (radio.frequency!=old_frequency)
    {
      ...
      SI5351.setFreq(0,radio.frequency + mode_delta);
    }
  }
  ...
  if (digitalRead(PIN_PTT)==LOW)
  {
    ...
    if (radio.mode==MODE_CW)
    {
      SI5351.setFreq(0,radio.frequency + CW_SIDETONE);
      delay(10);
    }
    else
    {
