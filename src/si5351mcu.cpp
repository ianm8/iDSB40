 ...
 *
 * Updates to avoid glitches across 40m and fast updates for
 * small frequency changes by Ian Mitchell, VK7IAN
 *
...
void __not_in_flash_func(Si5351mcu::setFreq)(uint8_t clk, uint32_t freq, const bool fast)
{
  ...
  if (force_div[clk])
  {
    // caller guarantees even + fvco in range
    outdivider = force_div[clk];
  }
  else
  {
...
void __not_in_flash_func(Si5351mcu::setFreqTX)(uint8_t clk, uint32_t freq)
{
  // Assumes setFreq() already programmed this clk nearby (divider cached, R==1 at HF)
  const uint32_t outdivider = omsynth[clk];
  const uint32_t fvco = outdivider * freq;
  const uint8_t  a = fvco / int_xtal;
  const uint32_t b = (fvco % int_xtal) >> 5;
  const uint32_t c = int_xtal >> 5;
  const uint32_t f = (128 * b) / c;
  const uint32_t MSNx_P1 = 128 * a + f - 512;
  const uint32_t MSNx_P2 = 128 * b - f * c;
  const uint8_t pll_stride = (clk > 0) ? 8 : 0;
  uint8_t regs[] = {
    (uint8_t)((MSNx_P1 & 0xFF00) >> 8),                             // 29
    (uint8_t)(MSNx_P1 & 0xFF),                                      // 30
    (uint8_t)(((c & 0x0F0000L) >> 12) | ((MSNx_P2 & 0x0F0000) >> 16)), // 31
    (uint8_t)((MSNx_P2 & 0xFF00) >> 8),                             // 32
    (uint8_t)(MSNx_P2 & 0xFF)                                       // 33
  };
  i2cWriteBurst(29 + pll_stride, regs, sizeof(regs));
}
